...
* You have verified which drive has failed by noting that the drive's left LED is blinking blue or using the Grid Manager to link:verify-component-to-replace.html[view the alert caused by the failed drive].
...
. Verify that the drive's left LED is blinking blue or use the drive slot ID from the alert to locate the drive.
+
