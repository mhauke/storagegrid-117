...
summary: 'This reference lists the default alerts that appear in the Grid Manager. As required, you can create custom alert rules to fit your system management approach.'
---
= Alerts reference
:icons: font
:imagesdir: ../media/
...
[.lead]
This reference lists the default alerts that appear in the Grid Manager. As required, you can create custom alert rules to fit your system management approach.
...
. Review the settings on the Grid Federation page for both the local and the remote grids. Confirm that all values are correct. See xref:../admin/grid-federation-create-connection.adoc[Create a grid federation connection].

. Review the certificates used for the connection. Make sure that certificates have not expired and that the details are valid.
. Confirm that all Admin and Gateway Nodes in the local and remote grids are online and available. Resolve any alerts that might be affecting these nodes and try again.
. If you provided a fully qualified domain name (FQDN) for the local or remote grid, confirm the DNS server is online and available.
