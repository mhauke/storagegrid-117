...
The IP address in textual representation will be unsupported in the future as an FQDN. Contact support if you need an IP address in a textual representation. See xref:../admin/configuring-s3-api-endpoint-domain-names.adoc[edit S3 endpoint domain names].