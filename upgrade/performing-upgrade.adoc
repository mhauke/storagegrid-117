...
. [[approval-step]]When you are ready to select grid nodes for upgrade (or if you need to remove selected nodes), use these instructions:
+
...
|Remove all selected nodes of the same type 
|Select the *Remove all* button for the node type

|Remove an individual node
|Select the *Remove* button for the node
...
* Stopping services
+
NOTE: You cannot remove a node when its Stage reaches *Stopping services*. The *Remove* button is disabled.
