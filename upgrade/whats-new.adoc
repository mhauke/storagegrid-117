...
* *EBKR: Empty Bucket Request*: Related to the new functionality for emptying a bucket, this message is generated when a user sends a request to turn Empty Bucket (also known as deleting bucket objects) on or off.
...
* *S3SL: S3 Select Request*: Related to the new S3 Select functionality, this message is generated when a user initiates an S3 Select job. 
