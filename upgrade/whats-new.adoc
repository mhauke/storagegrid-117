...
=== Grid federation and cross-grid replication
You can configure a grid federation connection between two StorageGRID systems to clone tenant account information and replicate bucket objects between the grids for disaster recovery and ransomware mitigation. See xref:../admin/grid-federation-overview.adoc[Use grid federation: Overview].

=== Delete all objects from bucket
Tenant users can now delete all objects in a bucket, so the bucket can be deleted. See xref:../tenant/delete-s3-bucket-objects.adoc[Delete all objects from bucket].
...
The following new audit message was added:
...
=== Volume restoration
Storage volume recovery lets you restore object data if a storage volume fails. For StorageGRID 11.7, volume restoration can be started using Grid Manager in addition to the existing method of using CLI commands. See xref:../maintain/restoring-volume.adoc[Restore volume using Grid Manager].
