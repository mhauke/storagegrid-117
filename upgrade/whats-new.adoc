...
Security settings have been added to help secure and protect your StorageGRID system. In the Grid Manager, select *CONFIGURATION* > *Security* > *Security settings*.

==== New TLS and SSH policies
The TLS and SSH policies tab allows you to create protocols and ciphers to establish secure TLS connections with client applications and secure SSH connections to internal StorageGRID services. See link:../admin/manage-tls-ssh-policy.html[Manage the TLS and SSH policy]

==== Object encryption and network security
The Network and objects tab allows you to set an encryption type for stored objects and select network security options. See link:../admin/changing-network-options-object-encryption.html[Configure network and object security].

==== Browser inactivity timeout
You can enable browser inactivity timeout for users of the Grid Manager and Tenant Manager. See link:../admin/changing-browser-session-timeout-interface.html[Change browser inactivity timeout].
