---
permalink: sg100-1000n/monitoring-node-encryption-in-maintenance-mode.html
sidebar: sidebar
 ...
 ** Key Management Server Details shows information about the KMS being used to encrypt the appliance. You can expand the server and client certificate sections to view certificate details and status.
  ...
  *** If there are unexpected problems connecting to KMS hosts, verify that the xref:checking-dns-server-configuration.adoc[domain name system (DNS) servers are correct] and that xref:../installconfig/configuring-network-links-sg100-and-sg1000.adoc[appliance networking is correctly configured].
