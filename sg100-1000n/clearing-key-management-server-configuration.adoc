---
permalink: sg100-1000n/clearing-key-management-server-configuration.html
sidebar: sidebar
...
xref:../maintain/grid-node-decommissioning.adoc[Decommission the node] to move any data it contains to other nodes in the grid.
...
After the appliance reboots and you have verified that KMS has been cleared and that the appliance in a pre-install state, you can physically remove the appliance from your StorageGRID system. See the xref:../maintain/preparing-appliance-for-reinstallation-platform-replacement-only.adoc[instructions for preparing the appliance for reinstallation].
