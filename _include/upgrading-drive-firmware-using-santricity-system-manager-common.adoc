...
NOTE: Drives that do not have redundancy must be updated using the offline method. This requirement includes any drive associated with SSD cache (including, for example, SSD drives in the SG6060), or any pool or volume group that is degraded. 
...
IMPORTANT: There are two types of drives in the SG6060: SSD and HDD. You must use the offline method to upgrade the drive firmware on the SSD drives. You may use the online method to upgrade the drive firmware on the HDD drives. 
