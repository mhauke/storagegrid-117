...
*Note*: The RAM value affects a node's actual metadata reserved space. See the link:../admin/managing-object-metadata-storage.html[description of what Metadata Reserved Space is] and link:../upgrade/increasing-metadata-reserved-space-setting.html[how to increase Metadata Reserved Space].
