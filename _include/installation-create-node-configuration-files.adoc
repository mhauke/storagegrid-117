...
*Note*: The RAM value affects a node's actual metadata reserved space. See the xref:../admin/managing-object-metadata-storage.adoc[description of what Metadata Reserved Space is] and xref:../upgrade/increasing-metadata-reserved-space-setting.adoc[how to increase Metadata Reserved Space].
...
|PORT_REMAP
|O
|Remaps any port used by a node for internal grid node communications or external communications. Remapping ports is necessary if enterprise networking policies restrict one or more ports used by StorageGRID, as described in xref:../network/internal-grid-node-communications.adoc[Internal grid node communications] or xref:../network/external-communications.adoc[External communications].
