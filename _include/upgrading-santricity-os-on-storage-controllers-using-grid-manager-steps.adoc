...
IMPORTANT: Be sure to apply the latest StorageGRID hotfix before you begin this procedure. See xref:storagegrid-hotfix-procedure.adoc[StorageGRID hotfix procedure]roo for details. 
...
[start=4]
. Review the information in the table for all of your appliances. Confirm that all storage controllers have *Nominal* status. If the status for any controller is *Unknown*, go to *Nodes* > *_appliance node_* > *Hardware* to investigate and resolve the issue.
