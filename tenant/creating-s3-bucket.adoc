...
+
If you enable S3 Object Lock for a bucket, bucket versioning is enabled automatically. You can optionally specify a default retention mode and default retention period for the bucket that are applied to each object ingested to the bucket that does not specify its own retention settings. See xref:configure-s3-object-lock-settings-for-bucket.adoc[Configure S3 Object Lock settings for a bucket].
. Select *Create bucket*.
+
