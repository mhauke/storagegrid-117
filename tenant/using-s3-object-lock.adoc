...
You can also specify a default retention mode and default retention period for the bucket. These are applied to each object added to the bucket that does not specify its own retention settings. See xref:creating-s3-bucket.adoc[Create S3 bucket] and xref:configure-s3-object-lock-settings-for-bucket.adoc[Configure S3 Object Lock for a bucket].
