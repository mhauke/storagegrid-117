...
.. Optionally, change the setting of *Replicate client deletes* to determine what happens on the other grid when an S3 client deletes an object from this bucket.
+
...
You can ingest objects into either bucket to test that cross-grid replication is configured correctly.

NOTE: For security reasons, S3 access keys are not cloned from the tenant's source grid to the tenant's destination grid. If you want to ingest objects into a bucket on the destination grid, you must create S3 access keys on the source grid and manually clone them to on the destination grid. See xref:grid-federation-clone-keys-with-api.adoc[Clone S3 access keys using the API]. 

. Using an S3 application, such as xref:use-s3-console.adoc[Experiment S3 Console], upload one or two small test objects to a bucket that is configured for cross-grid replication.
