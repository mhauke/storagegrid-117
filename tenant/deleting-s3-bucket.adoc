...
These instructions describe how to delete an S3 bucket using the Tenant Manager. You can also delete S3 buckets using the link:understanding-tenant-management-api.html[Tenant Management API] or the link:../s3/s3-rest-api-supported-operations-and-limitations.html[S3 REST API].

You cannot delete an S3 bucket if it contains objects, noncurrent object versions, or delete markers. For information about how S3 versioned objects are deleted, see link:../ilm/how-objects-are-deleted.html[How objects are deleted].
