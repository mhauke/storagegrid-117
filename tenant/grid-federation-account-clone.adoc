...
After you create (import) a federated group on the source grid, federated users can access the source grid. However, federated user can't sign in to the destination grid initially. Before a federated user can access the tenant account on the destination grid, you must create S3 access keys on the tenant's source grid and then manually clone the keys to the destination grid using the Tenant Management API. See <<manage-access-keys, Manage S3 access keys>>.
...
== [[manage-access-keys]]Manage S3 access keys
...
If you update or delete an existing tenant group, user, or access key on the source grid, StorageGRID will not clone the changes to the destination grid. If you need to keep this information in sync, you must perform the same operations on the destination grid.
