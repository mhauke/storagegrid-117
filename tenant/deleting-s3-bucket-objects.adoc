...
* If you use xref:grid-federation-manage-cross-grid-replication.adoc[cross-grid replication], note the following: 

** Using this option does not delete any objects from the bucket on the other grid.

** If you select this option for the source bucket, the *Cross-grid replication failure* alert will be triggered if you add objects to the destination bucket on the other grid. If you can't guarantee no one will add objects to the bucket on the other grid, xref:../tenant/grid-federation-manage-cross-grid-replication.adoc[disable cross-grid replication] for that bucket before deleting all bucket objects.
