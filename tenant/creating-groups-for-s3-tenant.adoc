...
NOTE: If your tenant account has the *Use grid federation connection* permission, any users you select when you create a local group on the source grid are not included when the group is cloned to the destination grid. For this reason, don't select users when you create the group. Instead, select the group when you create the users.
