...
+
IMPORTANT: If a local user needs to sign in to Tenant Manager on the destination grid, a user with Root access permission must sign in to Tenant Manager on the destination grid to add a password for the user on that grid. See link:../tenant/managing-local-users.html[Manage local users].
