...
+
If you selected the *Use grid federation connection* permission for the tenant, a new tenant with the same name and the same 20-digit account ID is automatically created on the other grid in the grid federation connection. You might need to wait up to 15 minutes for the new tenant to appear in Grid Manager on the other grid.
