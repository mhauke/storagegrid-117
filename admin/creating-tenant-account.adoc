...
. If you selected the *Use grid federation connection* permission for the tenant, wait up to 15 minutes for a new tenant with the same name, the same 20-digit account ID, and the same root user to be created on the other grid in the grid federation connection.
