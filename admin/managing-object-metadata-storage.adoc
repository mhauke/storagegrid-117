...
* The actual reserved space for metadata for SN1 is 6 TB. (The entire volume is reserved because volume 0 is smaller than the *Metadata Reserved Space* setting.)
* The allowed space for metadata on SN1 is 3 TB, based on the calculation shown in the <<actual-reserved-space-for-metadata,table for actual reserved space for metadata>>: (Actual reserved space for metadata − 1 TB) × 60%, up to a maximum of 3.96 TB.
...
* To learn how to monitor the object metadata capacity for each Storage Node, go to xref:../monitor/index.adoc[Monitor and troubleshoot].

* To increase the object metadata capacity for your system, add new Storage Nodes. Go to xref:../expand/index.adoc[Expand your grid].
