 ...
 ** *Subject* (optional): X.509 subject or distinguished name (DN) of the certificate owner. + 
 If no value is entered in this field, the subject for the generated certificate will be the first domain name or IP address.
...
You can revert to using the default S3 and Swift API certificate for S3 and Swift client connections to Storage Nodes. However, you cannot use the default S3 and Swift API certificate for a load balancer endpoint.
