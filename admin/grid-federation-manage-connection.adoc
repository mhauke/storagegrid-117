...
When you configure a grid federation connection, StorageGRID generates four security certificates to secure the connection in both directions. It creates a server certificate and the corresponding client certificate for the connection between grid 1 and grid 2 and a server certificate and the corresponding client certificate for the connection between grid 2 and grid 1.

image:../media/grid-federation-certificates.png[Grid federation certificates]

By default, all four certificates are valid for 730 days (two years). If these certificates expire, the connection will stop working and data will not longer be replicated between grids. 
...
. Use the Actions menu or the Details page, as follows:
+
[role="tabbed-block"]
====
...
. Download the updated verification package from the grid you are signed in to and provide it to the other grid.

. Sign in to the other grid, and select *Manage connection* > *Upload verification package*.

. Confirm that the *Connection status* is *Connected*.

. Securely delete all copies of the verification package from both grids.
+
[IMPORTANT]
This file contains secrets and must be deleted after you update the certificates.
