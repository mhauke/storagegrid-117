...
You can remove a grid federation connection from either grid in the connection. As shown in the figure, you must first confirm that the connection is not being used by any tenant on either grid.
...
+
IMPORTANT: If the connection is being used for cross-grid replication, you can't remove tenant permissions until replication has been stopped for every bucket on both sides of the connection.
