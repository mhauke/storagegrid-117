...
You can edit a connection by signing in to the primary Admin Node on either grid in the connection. After you make changes on the first grid, you must download a new verification file and upload it to the other grid.

. Sign in to the Grid Manager from the primary Admin Node on either grid.

. Select *CONFIGURATION* > *System* > *Grid federation*.

. Use the *Actions* menu on the Grid federation page or the details page for a specific connection.
+
[role="tabbed-block"]
====
...
. Provide the `_connection-name_.grid-federation` file to the grid admin for the other grid.
. From the other grid in the connection:
