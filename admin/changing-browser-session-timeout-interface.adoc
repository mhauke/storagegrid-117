...
NOTE: A separate, non-configurable timer for each user expires 16 hours after the user signs in. When this timer expires, the user is automatically signed out, even if the value for the browser inactivity timeout has not reached.
