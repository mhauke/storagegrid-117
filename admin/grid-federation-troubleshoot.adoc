...
In rare cases, you might see the message "`Tenant created without a clone`" after creating a new tenant with the *Use grid federation connection* permission. The issue occurs when the source grid appears to be *Connected* on the source grid even though the underlying connection is unhealthy. 
