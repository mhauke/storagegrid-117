...
. Ingest a new object into the bucket listed in the message and confirm that the error message does not reappear for that bucket.
+
NOTE: If the underlying issue still exists, the error will appear again 5 to 6 minutes after the new object is ingested.
...
.. Go link:grid-federation-retry-failed-replication.html[Identify and retry failed replication operations] to identify any objects or delete markers that failed to be replicated to the other grid and to retry replication as needed.
