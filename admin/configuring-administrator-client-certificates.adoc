...
+
If the subject does not contain a CN value, the CN is set to /CN=_first domain or IP_ if a domain or IP is present. Otherwise, it is set to /CN=StorageGRID or /CN StorageGRID client, based on the extendedKeyUsage for the generated certificate.
...
** *Subject* (optional): X.509 subject or distinguished name (DN) of the certificate owner.
+
The distinguished name of this certificate's entity. If the subject does not contain a CN value, the CN is set to /CN=_first domain or IP_ if a domain or IP is present. Otherwise, it is set to /CN=StorageGRID or /CN StorageGRID client, based on the extendedKeyUsage for the generated certificate.
