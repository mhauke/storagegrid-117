...
Each node in your grid has a unique node console password, which you need to log in to the node as “admin” using SSH, or to the root user on a VM/physical console connection. As needed, you can xref:change-node-console-password.adoc[change the node console password] for each node.
