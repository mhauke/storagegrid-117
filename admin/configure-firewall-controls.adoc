...
+
Use this tab to add IP addresses or subnets in CIDR notation that can access ports closed using the Manage external access tab. 
* Manage external access
+
Use this tab to close ports that are normally open, or reopen ports previously closed. 
...
The tab displays a field where you can enter IP addresses or subnets in CIDR notation you want to allow to access closed ports. Any previously configured addresses are also displayed. 

+
image::../media/privileged-address-list.png[Privileged address list]

. In the Address 1 field, enter the IP address of a client or subnet you want to allow access to closed ports. 
. Optionally, select *Add another IP address or subnet in CIDR notation* to add additional privileged clients. 
. Optionally, select the check box if you want to allow privileged IP addresses or subnets to access StorageGRID internal ports. Select *StorageGRID internal ports* to see documentation on the internal ports. 
+
...
NOTE: The configuration changes you make on the *Untrusted Client Network* tab override the settings on the *Manage external access* tab.  
