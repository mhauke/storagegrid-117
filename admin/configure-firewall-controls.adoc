...
For example, you could select *Untrust Client Network on displayed nodes* to add all nodes to the Untrusted Client Network list and then use the toggle besides an individual node to add that single node to the Trusted Client Network.
