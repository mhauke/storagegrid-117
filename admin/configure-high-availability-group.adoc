...
IMPORTANT: You cannot remove an HA group if it is bound to a load balancer endpoint. To delete an HA group, you must remove it from any load balancer endpoints that use it.
...
. If no load balance endpoints are listed, select the check box for each HA group you want to remove.
