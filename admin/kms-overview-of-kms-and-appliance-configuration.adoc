...
+
NOTE: You cannot enable the *Node Encryption* setting after an appliance is added to the grid, and you cannot use external key management for appliances that do not have node encryption enabled.
...
See link:../installconfig/optional-enabling-node-encryption.html[Enable node encryption] for details.
