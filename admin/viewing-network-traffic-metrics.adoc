...
* Write request rate by object size: The rate for PUT/POST/DELETE requests matching this policy. Hovering on an individual cell shows per second rates. Rates shown in the hover view are truncated to integer counts and might report 0 when there are non-zero requests in the bucket.
