...
You can configure a special proxy server address in SANtricity System Manager to cause AutoSupport messages to be transmitted through a StorageGRID Admin Node without the use of the appliance's management port. AutoSupport messages transmitted in this way are sent by whichever  Admin Node is acting as the preferred sender, and they use any link:../admin/configuring-admin-proxy-settings.html[Admin proxy settings] that have been configured in the Grid Manager.
