...
* Grid federation connections can occur on the Grid Network, Admin Network, or Client Network. The best practice is to use the Client Network to route the WAN traffic related to cross-grid replication and account clone. Using the Client Network helps to ensure that the Grid Network on each StorageGRID system remains isolated.
...
You must ensure that no node in either grid uses this port for other connections. For example, if you are using the Client Network on each grid for the grid federation connection, you can use the link:configure-firewall-controls.html#untrusted-client-network[Untrusted Client Network] feature to prevent the port used in the grid federation connection from being reached on nodes that don't expect grid federation traffic.
