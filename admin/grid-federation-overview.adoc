...
* Grid federation connections can occur on the Grid Network, Admin Network, or Client Network. However, the best practice is to use the Client Network on each grid. Using the Client Network to route the WAN traffic related to cross-grid replication and account clone helps to ensure that the Grid Network on each StorageGRID system remains isolated.

* In addition, you should prevent the port used in each grid federation connection from being reached on nodes that don't expect grid federation traffic. For instructions, see link:configure-firewall-controls.html#untrusted-client-network[Untrusted Client Network].
