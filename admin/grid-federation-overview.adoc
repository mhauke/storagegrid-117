...
* If you use identity federation for tenant groups and users, both grids must have access to the same federated groups and users. That is, you must configure the same LDAP server on both grids. If tenants use their own identity source, they will need to use the same LDAP server as well.
...
** When a tenant creates local groups, local users, or S3 access keys on the tenant's source grid, the local groups and local users are automatically cloned to the destination grid, and the S3 access keys are optionally cloned to the destination grid. 

** When a tenant imports a federated group on the tenant's source grid, assigns group permissions, and selects an S3 policy, the same federated group is imported into the tenant account on the destination grid. The permissions and S3 policy will be the same.
+
...
** When tenant users creates or imports groups, creates local users, or S3 access keys on the tenant's destination grid, these items are not cloned to the source grid.
+
