...
sidebar: sidebar
keywords: StorageGRID, S3 setup wizard, S3 application
...
[.lead]
You can use the S3 application setup wizard to configure StorageGRID for use with an S3 application. The setup wizard provides the objects the application needs to access a StorageGRID bucket.
...
You can create a new tenant and bucket for S3 application use. When creating a new tenant, if you select *Create root user S3 access key automatically*, the access key ID and secret access key for the tenant's root user are created automatically.
