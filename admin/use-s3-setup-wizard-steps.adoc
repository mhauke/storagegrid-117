...
For details about this task, see admin/creating-tenant-account.adoc[Create tenant account] and 
tenant/creating-s3-bucket.adoc[Create S3 bucket].
...
. For the tenant details step, enter the following information:
+
...
|*Name*
|Enter a name for the tenant account. For example, `S3 tenant`. 

|*Description* (optional)
|Enter a description that helps you identify the tenant.

|*Client type*
|By default client type is *S3* and is disabled.

|*Storage quota* (optional)
|If you want this tenant to have a storage quota, enter a numerical value for the quota and select the correct units.
...
.. Use *Enable object versioning* if you want to store each version of each object in this bucket. 
...
In the download data step, you can download the access key,  configuration values file and save it in a safe location. 
...
. Select I have reviewed these steps....

. Select Continue.

+

Check the option *I have reviewed these steps and understand what I need to do*.
