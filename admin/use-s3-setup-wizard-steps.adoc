...
You can create a new tenant and bucket for this S3 application to use, or you can select an existing tenant and bucket. If you create a new tenant, the system automatically creates the access key ID and secret access key for the tenant's root user.

For details about this task, see xref:creating-tenant-account.adoc[Create tenant account] and xref:../tenant/creating-s3-bucket.adoc[Create S3 bucket].
...
. For the *Enter details* step, enter the following information:
+
[cols="1a,2a" options="header"]
...
. Optionally, enter the following details to create a bucket for the tenant's objects, or select *Create tenant without bucket* to create the tenant without also creating a bucket.
...
.. Select *Enable object versioning* if you want to store every version of each object in this bucket. 

..  Select *Create tenant and bucket* to download data.
...
In the *Download data* step, you can download the access keys and a configuration file. 
...
. Select *Download configuration values* to download a `.txt` file containing details for the load balancer endpoint, tenant, bucket, and the root user. This file contains the values you need to connect the S3 application to the StorageGRID bucket. 

. . Save both files to a secure location.
+
IMPORTANT: Do not close this page until you have downloaded or copied both access keys and, optionally, downloaded the configuration file. The keys will not be available after you close this page. Make sure to save both files in a secure location because they can be used to obtain data from your StorageGRID system.
...
.  Select *Continue* to go to the summary.
