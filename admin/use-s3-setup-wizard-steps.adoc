...
NOTE: You must have Root access permission to access the wizard. Also make sure that you meet all the link:../admin/use-s3-setup-wizard.adoc[prerequisites] before using the wizard. 
...
.. Use *Create tenant only* to just create a tenant, else select *Create tenant and bucket* and go to the download S3 settings step.

== Step 4 of 6: Download S3 settings

In the download S3 settings step, you can download the access key file (if generated by the wizard) and the configuration values file and save them in a safe location. 
...
. Select the *I have successfully downloaded or copied both the access key ID and secret access key* check box to confirm that you have successfully downloaded and secured the access key file (if generated by the wizard) and the configuration values file.
