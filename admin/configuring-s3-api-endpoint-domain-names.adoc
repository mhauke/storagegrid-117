...
* Clients that connect to a load balancer endpoint, directly to a Storage Node, can customize the global S3 and Swift API certificate to include all required endpoint domain names.
 ...
 ** If clients connect to a load balancer endpoint that uses the global S3 and Swift API certificate, directly to Storage Nodes, update the global S3 and Swift API certificate.
