...
* *HTTP*: If you select the HTTP protocol, you must xref:configuring-admin-proxy-settings.adoc[create a proxy server] to convert AutoSupport messages to HTTPS. Messages sent using HTTP are rejected by NetApp’s AutoSupport servers. The HTTP protocol uses port 80. 
