...
* xref:use-s3-setup-wizard-steps.adoc[Use the S3 setup wizard]
* xref:../fabricpool/use-fabricpool-setup-wizard-steps.adoc[Use the FabricPool setup wizard]
...
If you plan to use S3 virtual hosted-style requests, the certificate must also include an *Alternative Name* entry for each xref:../admin/configuring-s3-api-endpoint-domain-names.adoc[S3 endpoint domain name] you have configured, including any wildcard names. For example:
...
NOTE: If you use wildcards for domain names, review the xref:../harden/hardening-guideline-for-server-certificates.adoc[Hardening guidelines for server certificates].
...
IMPORTANT: If the certificate used to secure the connection between the S3 application and StorageGRID expires, the application might temporarily lose access to StorageGRID. 
...
* *Node interfaces*. Client applications must use the IP address (or corresponding FQDN) of a selected Gateway or Admin Node and network interface. 
