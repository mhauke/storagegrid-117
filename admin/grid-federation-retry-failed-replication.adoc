...
An audit log entry is created for each object that StorageGRID failed to replicate to the destination bucket. Each entry contains the following information:
...
IMPORTANT: It might take 2 hours or more for an object or delete marker to be replicated to the other grid.
