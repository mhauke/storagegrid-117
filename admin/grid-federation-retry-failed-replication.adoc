...
. Search the `audit.log` for xref:../audit/cgrr-cross-grid-replication-request.adoc[CGRR (Cross-Grid Replication Request)] messages, and use the audit-explain tool to format the results.
+
...
The results of the command will look like this example, which has entries for six CGRR general errors. Of these, three errors are for "replicate object" operations and three errors are for "replicate delete marker" operations.
