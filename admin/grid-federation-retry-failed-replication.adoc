...
. Search the `audit.log` for link:../audit/cgrr-cross-grid-replication-request.html[CGRR (Cross-Grid Replication Request)] messages, and use the audit-explain tool to format the results.
+
...
The results of the command will look like this example, which has entries for six CGRR messages. In the example, all cross-grid replication requests returned in a general error because the object could not be replicated. Three errors are for "replicate object" operations, and three errors are for "replicate delete marker" operations.
...
. In the Parameter section, enter the version ID for the cross-grid-replication request you want to check.

. Select *Execute*.

. Confirm that the server response code is *200* and that the replicationStatus is PENDING. 
+
NOTE: A pending cross-grid replication request is a request that has been added to the internal queue for processing. It might take 2 hours or more for replicationStatus to change to COMPLETED.
