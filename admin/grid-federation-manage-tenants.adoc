...
* When the new tenant is created on the first grid, a new tenant is automatically created on the other grid in the connection.

* You might need to wait up to 15 minutes for the automatically created tenant to appear in Grid Manager on the other grid.

* The tenants on both grids will have the same 20-digit account ID, name, description, quota, and permissions. However, for security reasons, the password for a local root user is not copied to the automatically created tenant.  

* Before a local root user can access the automatically created tenant, a grid administrator for that grid must xref:changing-password-for-tenant-local-root-user.adoc[change the password for the local root user].
...
** (Recommended.) Sign in to the Tenant Manager and stop replication (permanently disable replication) for each of the tenant's buckets. Then, repeat the steps to remove the permission.
