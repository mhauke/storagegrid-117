...
summary: 'You can check and temporarily change the DNS servers that are currently in use by this appliance node.'
---
...
You might need to change the DNS server settings if an encrypted appliance can't connect to the key management server (KMS) or KMS cluster because the hostname for the KMS was specified as a domain name instead of an IP address. Any changes that you make to the DNS settings for the appliance are temporary and are lost when you exit maintenance mode. To make these changes permanent, specify the DNS servers in Grid Manager (*MAINTENANCE* > *Network* > *DNS servers*).
