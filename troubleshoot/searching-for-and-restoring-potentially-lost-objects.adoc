...
.What you'll need
* You must have the UUID of any lost object, as identified in xref:investigating-lost-objects.adoc[Investigate lost objects].
...
When you are logged in as root, the prompt changes from `$` to `#`.
 ...
 .. Use grep to extract the xref:../audit/object-ingest-transactions.html.adoc[audit messages associated with the potentially lost object] and send them to an output file. Enter: `grep uuid-valueaudit_file_name > output_file_name`
+
For example:
+
----
...
. xref:resetting-lost-and-missing-object-counts.adoc[Reset the lost and missing object counts] in the Grid Manager.
