...
Check the ciphers that are offered by the external system. The system must use one of the xref:../admin/supported-ciphers-for-outgoing-tls-connections.adoc[ciphers supported by StorageGRID] for outgoing TLS connections, as shown in the instructions for administering StorageGRID.
...
. xref:../commonhardware/changing-mtu-setting.adoc[Modify the MTU settings] as necessary to ensure they are the same for the Grid Network interface (eth0) on all nodes.
...
*Note*: These steps are applicable only for NRER errors caused by FEC mismatch on StorageGRID appliances.
  ...
  +
  * xref:../sg6000/changing-link-configuration-of-sg6000-cn-controller.adoc[SG6000]
  * xref:../sg5700/changing-link-configuration-of-e5700sg-controller.adoc[SG5700]
  * xref:../sg5600/changing-link-configuration-of-e5600sg-controller.adoc[SG5600]
  * xref:../sg100-1000/changing-link-configuration-of-services-appliance.adoc[Sg100 and SG1000]
  ... Then, change the FEC settings on the switch ports. The StorageGRID appliance ports will adjust their FEC settings to match, if possible.
+
...
NOTE: When xref:../maintain/configuring-ntp-servers.adoc[specifying the external NTP source] for a production-level StorageGRID installation, do not use the Windows Time (W32Time) service on a version of Windows earlier than Windows Server 2016. The time service on earlier versions of Windows is not sufficiently accurate and is not supported by Microsoft for use in high-accuracy environments, such as StorageGRID.
...
In some cases, network issues can be resolved by using MAC address cloning. If you are using virtual hosts, set the value of the MAC address cloning key for each of your networks to "true" in your node configuration file. This setting causes the MAC address of the StorageGRID container to use the MAC address of the host. To create node configuration files, see the instructions for xref:../rhel/creating-node-configuration-files.adoc[Red Hat Enterprise Linux or CentOS] or xref:../ubuntu/creating-node-configuration-files.adoc[Ubuntu or Debian].
...
For more information on enabling MAC cloning, see the instructions for xref:../rhel/configuring-host-network.adoc[Red Hat Enterprise Linux or CentOS] or xref:../ubuntu/configuring-host-network.adoc[Ubuntu or Debian].
...
For more information about using Promiscuous Mode, see the instructions for xref:../rhel/configuring-host-network.adoc[Red Hat Enterprise Linux or CentOS] or xref:../ubuntu/configuring-host-network.adoc[Ubuntu or Debian].
