...
If you experience an error when you are signing in to a StorageGRID Admin Node, your system might have an issue with the xref:../admin/using-identity-federation.adoc[identity federation configuration], a xref:../troubleshooting-network-hardware-and-platform-issues.adoc[networking] or xref:../installconfig/troubleshooting-hardware-installation.adoc[hardware] problem, an issue with xref:../admin/what-admin-node-is.adoc[Admin Node services], or an xref:../maintain/recovering-failed-storage-volumes-and-rebuilding-cassandra-database.adoc[issue with the Cassandra database] on connected Storage Nodes.
...
+
Provide the logs you collected to technical support. See also xref:../monitor/logs-files-reference.adoc[Log files reference].
