...
You can xref:../monitor/monitoring-storage-capacity.adoc#monitor-object-metadata-capacity-for-each-storage-node[monitor object metadata capacity for each Storage Node] to help you anticipate errors and correct them before they occur.
...
You can xref:../monitor/running-diagnostics.adoc[run diagnostics] to obtain additional information on the current state of your grid.
