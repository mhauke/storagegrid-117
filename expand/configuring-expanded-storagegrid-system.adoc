...
* If you added a Storage Node to an existing site, confirm that the storage grade you selected is associated with a storage pool that is used in an ILM rule. See xref:../ilm/creating-and-assigning-storage-grades.adoc[Create and assign storage grades].
