...
* If you added a site, xref:../ilm/creating-storage-pool.adoc[create a storage pool] for the site and each storage grade you selected for the new Storage Nodes. Confirm that the ILM policy meets the new requirements. If rule changes are required, link:../ilm/access-create-ilm-rule-wizard.html[create new rules] and link:../ilm/creating-proposed-ilm-policy.html[update the ILM policy]. If the rules are already correct, link:../ilm/activating-ilm-policy.html[activate a new policy] with no rule changes to ensure StorageGRID uses the new nodes.
