...
* Confirm that Network Time Protocol (NTP) servers are accessible from that site. See xref:../maintain/configuring-ntp-servers.adoc[Manage NTP servers].
+
...
* xref:../ilm/viewing-storage-pool-details.adoc[View storage pool details] to confirm that each node you added is included in the expected storage pools and used in the expected ILM rules.
