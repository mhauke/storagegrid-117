...
* Redistributes erasure-coded data evenly between Storage Node without considering the maximum capacity of each node.

* Will likely decrease the performance of ILM operations and S3 and Swift client operations when it runs&#8212; redistributing erasure-coding fragments requires additional resources.
...
** The EC rebalance procedure does not rebalance replicated object copies. Replicated copies will continue to consume space on the existing nodes.
