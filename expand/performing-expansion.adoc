...
+
NOTE: While approving expansion nodes, if you discover that two nodes have duplicate IP addresses on the Grid Network, you must cancel the expansion, redeploy one of the virtual machines or appliances with a non-duplicate IP, and restart the expansion.
