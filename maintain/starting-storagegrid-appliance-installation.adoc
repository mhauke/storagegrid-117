...
. In the Installation section, confirm that the current state is "`Ready to start installation of `_<node name>_` into grid with Primary Admin Node `_admin_ip_``" and that the *Start Installation* button is enabled.
+
