...
+
In rare cases, such as when instructed by technical support, or when you know that the replacement node has fewer volumes available for object storage than the original node, you must link:restoring-object-data-to-storage-volume-if-required.html[restore object data manually]. If one of these cases applies, answer `y`.
+
