...
sidebar: sidebar
keywords: recover, restore node to host, validate node, start host service
...
. For grid nodes that do not have a `/var/local` volume, re-create the node's configuration file to restore it to the host.
+
See the instructions for link:../rhel/creating-node-configuration-files.html[Red Hat Enterprise Linux or CentOS] or link:ubuntu/creating-node-configuration-files.html[Ubuntu or Debian].
+
...
. Force the node into recovery mode:
