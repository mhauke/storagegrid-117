 ...
 ** Details about each node:
  ...
  *** For Admin Nodes and Gateway Nodes, whether the node is currently used in a high availability (HA) group. You cannot decommission an Admin Node or a Gateway Node that is used in a HA group. Before you start the decommission, edit HA groups to remove all nodes at the site or remove the HA group if it only includes nodes from this site. For instructions, see link:../admin/managing-high-availability-groups.html[Manage high availability (HA) groups].
