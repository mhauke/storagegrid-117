...
+
. After you replace the storage, make sure you rescan or reboot to make sure that it is recognized by the operating system. In the Grid Manager, select *NODES* > `*Storage Node*` > *Hardware* > *StorageGRID Appliance* > *Storage RAID mode* to ensure the RAID is healthy before proceeding.
+
