...
. Deploy a new virtual machine to be the replacement node, and connect it to one or more StorageGRID networks. For instructions see xref:../vmware/deploying-storagegrid-node-as-virtual-machine.adoc[Deploying a StorageGRID node as a virtual machine].
+
