...
* If you decommissioned an appliance node and the data on the appliance was protected using node encryption, use the StorageGRID Appliance Installer to clear the key management server configuration (Clear KMS). You must xref:../commonhardware/monitoring-node-encryption-in-maintenance-mode.adoc[clear the KMS configuration] if you want to add the appliance to another grid.
