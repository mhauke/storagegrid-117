...
+
NOTE: If a volume goes offline (is unmounted) before an online node decommission starts, the decommission will not start and an error displays.
...
* If you decommissioned an appliance node and the data on the appliance was protected using node encryption, use the StorageGRID Appliance Installer to clear the key management server configuration (Clear KMS). You must xref:../commonhardware/monitoring-node-encryption-in-maintenance-mode.adoc[clear the KMS configuration] if you want to use the appliance in another grid.
