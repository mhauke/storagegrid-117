...
. For the Define root access step, specify which user will have the initial Root access permission for the tenant account, based on whether your StorageGRID system uses link:using-identity-federation.html[identity federation], link:configuring-sso.html[single sign-on (SSO)], or both. 
+
