...
You can use the FabricPool wizard to configure StorageGRID as the object storage system for a FabricPool cloud tier. After you complete the setup wizard, you can enter the required details into ONTAP System Manager.
...
NOTE: These instructions describe how to use the FabricPool setup wizard to configure StorageGRID for use with FabricPool. If you want to configure StorageGRID for use with any other S3 client application, go to xref:../admin/use-s3-setup-wizard.adoc[Use S3 setup wizard].

== How to create connections for more than one cluster

If you plan to connect the same StorageGRID system to multiple ONTAP systems, use the FabricPool setup wizard to create an HA group, load balancer endpoint, tenant, and bucket for the first cluster.
...
If you upgraded to StorageGRID 11.7 from a previous StorageGRID version, confirm that you have configured the storage pool you will use. In general, you should create a storage pool for each StorageGRID site you will use to store ONTAP data. 
