...
If you plan to use the StorageGRID load balancer, you have reviewed the general xref:../admin/managing-load-balancer-endpoints.adoc[considerations for load balancing] and the xref:best-practices-for-load-balancing.adoc[best practices for load balancing for FabricPool]. You have the certificates you will upload or the values you need to generate a certificate.
