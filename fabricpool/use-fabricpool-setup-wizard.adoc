...
NOTE: If you have ONTAP System Manager version 9.12.1 or later, you can upload the settings directly. If you have an earlier version, you can enter the same values manually. 

You can complete the FabricPool and S3 setup wizard when you first sign in to the StorageGRID Grid Manager, or you can access and complete the wizard at any later time. Depending on your requirements, you can also configure some or all of the required items manually and then use the wizard to assemble the values that ONTAP needs into a single file.
...
+
The certificate must include the domain name that ONTAP will use for StorageGRID. 
