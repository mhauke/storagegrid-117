...
*Note*: Leave the *Add key usage extensions* check box selected. ONTAP requires certificates to include these extensions.
...
. Go to link:configure-dns-server.html[Configure the DNS server] and ensure that the domain name system (DNS) includes a record to associate the StorageGRID server name (fully qualified domain name) to each StorageGRID IP address you will use. 
