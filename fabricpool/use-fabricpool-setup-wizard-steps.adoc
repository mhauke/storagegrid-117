...
+
NOTE: Do not select a bucket that has S3 Object Lock enabled for use with FabricPool.
...
+
CAUTION: Do not close this page until you have copied both access keys, downloaded the ONTAP settings, or both. The keys will not be available after you close this page. Make sure to save this information in a secure location because it can be used to obtain data from your StorageGRID system.

. Select the check box to confirm you have downloaded or copied the access key ID and secret access key.
...
+
As soon as FabricPool begins tiering data to StorageGRID, you can go to the Traffic Classification Policies page to view network traffic metrics for this policy. Later, you can also add rules to limit other workloads and ensure that the FabricPool workload has the majority of bandwidth. 
