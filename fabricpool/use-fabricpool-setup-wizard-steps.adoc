...
| Use 2+1 erasure coding from day 0 to forever
...
CAUTION: To avoid data loss, never use an ILM rule that will expire or delete FabricPool cloud tier data. Set the retention period to *forever* to ensure that FabricPool objects aren't deleted by StorageGRID ILM.
