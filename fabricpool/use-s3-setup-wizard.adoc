...
The S3 setup wizard guides you through each step of configuring StorageGRID for use with FabricPool. As part of completing the wizard, you download a file that you can use to enter values into ONTAP System Manager. The wizard helps you to configure your system more quickly and helps to make sure your settings conform to the current best practices for StorageGRID and FabricPool. 

You can complete the S3 setup wizard the very first time you sign in to the StorageGRID Grid Manager or you can access and complete the wizard at any later time. Depending on your requirements, you can also configure some or all of the required items manually and then use the wizard to assemble the values that ONTAP needs into a single file.

Use the wizard unless you know you have special requirements or your implementation will require significant customization. For example, you cannot use the S3 Setup Wizard if S3 Object Lock is enabled for your grid. Instead, you must configure each item manually in StorageGRID. Start by xref:creating-ha-group-for-fabricpool.adoc[creating an HA group].
...
* You have reviewed the best practices for xref:best-practices-for-high-availability-groups.adoc[high availability groups], xref:best-practices-for-load-balancing.html.adoc[load balancing], and the xref:best-practices-ilm.adoc[ILM rules and policy].
...
. To use the StorageGRID Load Balancer service, select *Create endpoint*, or select an existing endpoint. To use an external load balancer, select the *External load balancer* tab. 
+
[role="tabbed-block"]
====
...
. Select *Download* to save the values you've entered so far and the access keys to a text file that you can upload or enter into ONTAP System Manager. 
+
...
*	Tenant account ID, root username, root user password, access key ID and secret access key
*	Bucket name 
...
The S3 setup wizard automatically creates the recommended ILM rule for FabricPool use. This rule applies only to the bucket you specified. It uses 2+1 erasure coding to store the data that is tiered from ONTAP. 
...
After the S3 setup wizard creates the ILM rule for FabricPool use, it creates a proposed ILM policy to use the new rule. The new proposed policy is a clone of the current active policy. The new FabricPool ILM rule is added at the top and will be the first rule evaluated. 
