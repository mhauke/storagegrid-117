...
.. Select *Create bucket*.
+
NOTE: Don't select *Enable object versioning* for the FabricPool bucket. Similarly, do not edit a FabricPool bucket to use *Available* or a non-default consistency level. The recommended bucket consistency level for FabricPool buckets is *Read-after-new-write*, which is the default setting for a new bucket. 
