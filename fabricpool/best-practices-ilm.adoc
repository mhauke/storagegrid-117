...
+
CAUTION: To avoid data loss, never use an ILM rule that will expire or delete FabricPool cloud tier data. Set the retention period in each ILM rule to *forever* to ensure that FabricPool objects aren't deleted by StorageGRID ILM.
