...
IMPORTANT: FabricPool has no knowledge of StorageGRID ILM rules or policies. Data loss can occur if the StorageGRID ILM policy is misconfigured. For detailed information, see link:../ilm/what-ilm-rule-is.html[Create an ILM rule: Overview] and link:../ilm/creating-ilm-policy.html[Create an ILM policy: Overview].
