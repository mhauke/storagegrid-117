...
IMPORTANT: FabricPool has no knowledge of StorageGRID ILM rules or policies. Data loss can occur if the StorageGRID ILM policy is misconfigured. For detailed information, see xref:../ilm/what-ilm-rule-is.adoc[ILM rules] and xref:../ilm/what-ilm-policy-is.adoc[ILM policies].
...
== Best practices for using ILM with FabricPool
