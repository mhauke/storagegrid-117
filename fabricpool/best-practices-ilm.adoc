...
* Don't use an ILM rule that will expire or delete FabricPool cloud tier data. Set the retention period in each ILM rule to "`forever`" to ensure that FabricPool objects are not deleted by StorageGRID ILM.
* Don't create rules that will move FabricPool cloud tier data out of the bucket to another location. You cannot use a Cloud Storage Pool to move FabricPool data to another object store. Similarly, you cannot archive FabricPool data to tape using an Archive Node. 
+
