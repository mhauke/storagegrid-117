...
The only requirement for tiering data from multiple ONTAP clusters to a single StorageGRID system is that you must use a different bucket for each aggregate. Ideally, you should use a different tenant for each cluster. Based on your requirements, you can use the same high availability (HA) group, load balancer endpoint, and ILM rule for all clusters, or you can configure each of these items for each cluster.
