...
To prevent service interruptions, confirm that all other Storage Nodes are connected to the grid before starting the Fibre Channel HBA replacement or replace the adapter during a scheduled maintenance window when periods of service disruption are normally expected. See the information about xref:../monitor/monitoring-node-connection-states.adoc[determining node connection states].
...
See information about xref:../ilm/why-you-should-not-use-single-copy-replication.adoc[why you should not use single-copy replication].
...
xref:reinstalling-fibre-channel-hba.adoc[Install the replacement Fibre Channel HBA].
