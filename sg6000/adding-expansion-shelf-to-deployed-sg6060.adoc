...
+
The green checkmark icon means that no alerts are active and the node is connected to the grid. For a description of node icons, see link:../monitor/monitoring-node-connection-states.html.html[Monitor node connection states].
