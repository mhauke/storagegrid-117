...
To replace the battery in the storage controller, see the steps in the instructions for link:replacing-storage-controller-sg6000.html[replacing a storage controller]. Those instructions describe how to remove a controller from the appliance, remove the battery from the controller, install the battery, and replace the controller.
...
link:replacing-storage-controller-sg6000.html[Replace storage controller]
