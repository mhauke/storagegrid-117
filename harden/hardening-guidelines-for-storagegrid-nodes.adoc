...
Don't allow untrusted users to access StorageGRID nodes over the network when the nodes are being installed. Nodes are not fully secure until they have joined the grid.
...
* Don't enable outbound services for untrusted tenants. For example, when creating the account for an untrusted tenant, do not allow the tenant to use its own identity source and do not allow the use of platform services. See the steps for link:../admin/creating-tenant-account.html[creating a tenant account].
...
* If the storage controller in the appliance includes FDE or FIPS drives and the Drive Security feature is enabled, use SANtricity to configure Drive Security keys. See link:../installconfig/accessing-and-configuring-santricity-system-manager.html[Configure SANtricity System Manager (SG6000 and SG5700)].
* For appliances without FDE or FIPS drives, enable node encryption using a Key Management Server (KMS). See link:../installconfig/optional-enabling-node-encryption.html[Optional: Enable node encryption].