...
For a list of all internal and external ports used by StorageGRID, see xref:../installconfig/reviewing-appliance-network-connections.adoc[Review appliance network connections].
...
* Optionally, use an Admin proxy for more control over AutoSupport communication from Admin Nodes to NetApp support. See the steps for xref:../admin/configuring-admin-proxy-settings.adoc[creating an Admin proxy].
...
For more information, see the instructions for xref:../admin/index.adoc[administering StorageGRID].
...
* Do not enable outbound services for untrusted tenants. For example, when creating the account for an untrusted tenant, do not allow the tenant to use its own identity source and do not allow the use of platform services. See the steps for xref:../admin/creating-tenant-account.adoc[creating a tenant account].
...
* Optionally, use a Storage proxy for more control over Cloud Storage Pools and platform services communication from Storage Nodes to external services. See the steps for xref:../admin/configuring-storage-proxy-settings.adoc[creating a Storage proxy].
...
* Configure and use load balancer endpoints. See xref:../admin/managing-load-balancing.adoc[Considerations for load balancing].
...
* xref:../sg6100/index.adoc[SG6000 storage appliances]

* xref:../sg6000/index.adoc[SG1000 storage appliances]
