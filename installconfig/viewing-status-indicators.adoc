...
* To see all other hardware and environmental diagnostic information, use the hardware diagnostics in SANtricity OS.
+
This includes looking for hardware issues such as power, temperature, and disk drives. The appliance relies on SANtricity OS to monitor all platform environmental statuses.
...
HE
a|
The system is synchronizing with SANtricity OS.
a|
HC
a|
