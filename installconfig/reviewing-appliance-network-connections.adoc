...
[.lead]
Before installing the StorageGRID appliance, you should understand which networks can be connected to the appliance and how the ports on each controller are used. StorageGRID network requirements are fully explained in the xref:../network/index.adoc[Networking guidelines].
...
NOTE: For detailed information about StorageGRID networks, see the xref:../network/storagegrid-network-types.adoc[StorageGRID network types].
