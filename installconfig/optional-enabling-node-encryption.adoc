...
Review the information about xref:../admin/kms-configuring.adoc[configuring KMS].
...
You can view node-encryption status, KMS details, and the certificates in use when the appliance node is in maintenance mode. See xref:../commonhardware/monitoring-node-encryption-in-maintenance-mode.adoc[Monitor node encryption in maintenance mode] for information.
