...
IMPORTANT: Data that exists on an appliance that has node encryption enabled before it is connected to the KMS is encrypted with a temporary key that is not secure. The appliance is not protected from removal or theft until the key is set to a value provided by the KMS.
...
+
Prior to appliance installation you can clear *Enable node encryption* without risk of data loss. When the installation begins the appliance node accesses the KMS encryption keys in your StorageGRID system and begins disk encryption. You aren't able to disable node encryption after the appliance is installed.
+
