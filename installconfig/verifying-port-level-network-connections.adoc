...
Additionally, you can test connectivity on the Admin and Client Networks and on UDP ports, such as those used for external NFS or DNS servers. For a list of these ports, see the xref:../network/network-port-reference.adoc[network port reference].
