...
. To apply the configuration from the JSON file to the appliance, enter the following where `_SGA-install-ip_` is the Admin Network IP address of the appliance, `_json-file-name_` is the name of your JSON file, and `_node-name-inside-json-file_` is the name of the primary Admin Node: +
`configure-sga.py advanced --restore-file _json-file-name_ --restore-node _node-name-inside-json-file_ _SGA-install-ip_`

. To confirm the current configuration of the appliance node, enter the following where `_SGA-install-ip_` is the Admin Network IP address of the appliance: +
