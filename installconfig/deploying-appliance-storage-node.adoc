...
* If you are cloning an appliance node, continue following the xref:../commonhardware/appliance-node-cloning-procedure.adoc[appliance node cloning] process.
...
** To add an appliance Storage Node to an existing StorageGRID system, see the instructions for xref:../expand/adding-grid-nodes-to-existing-site-or-adding-new-site.adoc[adding grid nodes].
** To deploy an appliance Storage Node as part of a recovery operation, see instructions xref:../maintain/recovering-storagegrid-appliance-storage-node.adoc[recovering an appliance Storage Node].
...
+
The name that appears here will be the appliance node's system name. System names are required for internal StorageGRID operations and cannot be changed.
...
xref:../commonhardware/appliance-node-cloning-procedure.adoc[node cloning procedure].
