...
.. Enter: `*ipmitool lan print*` to display the current BMC configurations for the appliance.
...
.. To restore the BMC configuration to use a static IP, netmask, and gateway, log into the appliance as root and enter the following commands:
+
`*ipmitool lan set 1 ipaddr _Appliance_IP_*`
+
`*ipmitool lan set 1 netmask _Netmask_IP_*`
+
`*ipmitool lan set 1 defgw ipaddr _Default_gateway_*`
