...
| HTTPS
| S3 clients
| Load balancer endpoints on Admin Nodes and Gateway Nodes
| 10443 is the default port when you create the first load balancer endpoint and increments for each additional load balancer endpoint you create. However, a load balancer endpoint can use any unused external port. 
...
| TCP
| HTTPS
| Grid federation connections on all nodes
| Grid federation connections on all nodes
| These ports are reserved for any grid federation connections you might create. Both grids in a given connection must use the same port.
