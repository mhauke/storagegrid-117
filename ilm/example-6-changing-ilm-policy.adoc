...
In this example, a new StorageGRID site has been added in an expansion, and a new active ILM policy needs to be implemented to store data at the new site. To implement a new active policy, first create a proposed policy by either xref:xref:creating-proposed-ilm-policy.adoc[cloning an existing policy _or_ starting from scratch]. Afterward, you must [simulate] and then [activate] the new policy.
