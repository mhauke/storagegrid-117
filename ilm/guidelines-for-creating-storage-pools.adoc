...
* For StorageGRID 11.6 and earlier, a default storage pool, All Storage Nodes, is created during installation. It uses the default site, All Sites, and the storage grade, *includes all storage grades*. The All Storage Nodes storage pool is automatically updated whenever you add new data center sites.
+
