...
* In general, do not use the All Storage Nodes storage pool (StorageGRID 11.6 or earlier) or the All Sites site. These items are automatically updated to include any new sites you add in an expansion, which might not be the behavior you want. Before using the All Storage Nodes storage pool or the All Sites site, carefully review the guidelines for creating storage pools used for replicated and erasure-coded copies.
