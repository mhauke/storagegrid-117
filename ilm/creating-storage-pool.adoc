...
NOTE: When you install StorageGRID 11.7 on a new grid, storage pools are automatically created for each site to reduce the number of steps required to create new ILM rules. Site-specific storage pools are not created during upgrades to StorageGRID 11.7.
...
+
The Storage Pools page contains two tabs: one for Storage pools and one for Cloud Storage Pools.
+
The Storage pools tab lists:
+
* All defined storage pools
* The system-default storage pool, All Storage Nodes, which uses the system-default site, All Sites
* The default storage grade, All Storage Nodes
+
IMPORTANT: The All Storage Nodes storage pool is automatically updated whenever you add new data center sites. Using this storage pool in ILM rules is not recommended.
