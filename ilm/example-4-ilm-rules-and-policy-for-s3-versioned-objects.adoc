...
NOTE: To perform ILM policy simulation on a noncurrent version of an object, you must know the object version's UUID or CBID. To find the UUID and CBID, use xref:verifying-ilm-policy-with-object-metadata-lookup.adoc[object metadata lookup] while the object is still current.
...
+
NOTE: To simulate noncurrent object versions, you must use that version's UUID or CBID. While the object is still current, you can use xref:verifying-ilm-policy-with-object-metadata-lookup.adoc[object metadata lookup] to find its UUID and CBID.
