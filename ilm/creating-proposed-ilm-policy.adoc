...
NOTE: If the global S3 Object Lock setting has been enabled, you must ensure that the ILM policy is compliant with the requirements of buckets that have S3 Object Lock enabled. Follow the steps labeled, _"S3 Object Lock enabled"_.
...
+
Select a rule name to view the settings for that rule.
+
...
+
NOTE: If no rule is listed for the Select a default rule step of the wizard, you must exit the Create a proposed policy window and xref:creating-default-ilm-rule.adoc[create a default ILM rule].
+
...
//end Cloned policy, begin S3 Obj Lock enabled
...
+
The Create a proposed policy window now lists the rules you selected. The default rule is at the end, with the other rules above it. If S3 Object Lock is enabled and you also selected a non-compliant "`default`" rule, that rule is added as the second-to-last rule in the policy.
+
