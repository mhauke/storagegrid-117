...
. xref:creating-proposed-ilm-policy.adoc[Create a proposed policy].
...
.. In the Select other rules step, select the check box for the new rule, unselect the check box for the original rule, and select *Select*.
...
.. Select the delete icon image:../media/icon-x-to-remove.png[Remove icon] to remove the incorrect rule, and select *Save*.
.. Select *ILM* > *Rules*.
