...
* Only use the system-provided policy, Baseline 2 copies policy, in test systems. The Make 2 Copies rule in this policy uses the All Storage Nodes storage pool (StorageGRID 11.6 and earlier), which contains all sites. If your StorageGRID system has more than one site, two copies of an object might be placed on the same site.
+
