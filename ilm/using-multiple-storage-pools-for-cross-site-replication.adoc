...
summary: 'If your StorageGRID deployment includes more than one site, you can use multiple storage pools, replication, and erasure coding to enable site-loss protection.'
---
...
[.lead]
If your StorageGRID deployment includes more than one site, you can use multiple storage pools, replication, and erasure coding to enable site-loss protection.

Replication and erasure coding use storage pools differently. To use replication for site-loss protection, xref:guidelines-for-creating-storage-pools.adoc#guidelines-for-storage-pools-used-for-replicated-copies[create storage pools that consist of only one site each]. To use erasure coding for site-loss protection, xref:guidelines-for-creating-storage-pools.adoc#guidelines-for-storage-pools-used-for-erasure-coded-copies[create storage pools that consist of multiple sites].

Then specify multiple storage pools in the xref:create-ilm-rule-define-placements.adoc[ILM rule’s placement instructions]. For replicated copies, one copy of each object will be placed at each site. For erasure-coded copies, 

//. Alternatively, configure an ILM rule to make two replicated copies and specify three storage pools.

//The copies are distributed to balance disk usage among the storage pools, while ensuring that the two copies are stored at different sites.
