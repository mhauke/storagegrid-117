...
When you add a Storage Node in an expansion, you can select which storage grade will be used for the new node. If an ILM rule already uses the storage grade you selected, the new node can be used immediately after the expansion completes. Otherwise, you must assign the storage grade to a storage pool and add the storage pool to an ILM rule.
