...
If storage grade is not a concern (for example, all Storage Nodes are identical), you can skip this procedure and use the *All Storage Nodes* selection for the storage grade when you xref:creating-storage-pools.adoc[create storage pools]. Using this selection ensures that the storage pool will include every Storage Node at the site, regardless of its storage grade.
