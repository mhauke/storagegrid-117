...
[.lead]
Step 3 (Define placements) of the Create ILM Rule wizard allows you to define the placement instructions that determine how long objects are stored, the type of copies (replicated or erasure coded), the storage location, and the number of copies.
...
. Select *Continue*.
+
xref:create-ilm-rule-select-ingest-behavior.adoc[Step 3 (Select ingest behavior)] appears.
