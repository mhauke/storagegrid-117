...
*If you specify more than one storage pool*, keep these rules in mind:
  ...
  *** If the storage pools overlap (contain the same Storage Nodes), all copies of the object might be saved at only one site. For this reason, do not specify the All Storage Nodes storage pool (StorageGRID 11.6 and earlier) and another storage pool.
...
+
NOTE: See link:enable-site-loss-protection.html[Enable site-loss protection.]
+
