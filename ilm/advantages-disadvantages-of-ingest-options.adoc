...
NOTE: Before selecting a consistency level, read the full description of consistency controls in the instructions for link:../s3/consistency-controls.html[S3] or link:../swift/storagegrid-swift-rest-api-operations.html[Swift] client applications. You should understand the benefits and limitations before changing the default value.
