...
If an object has been stored in a xref:../ilm/what-cloud-storage-pool-is.adoc[Cloud Storage Pool], the behavior of a GET Object request depends on the state of the object. See xref:head-object.adoc[HEAD Object] for more details.
...
Use a xref:post-object-restore.adoc[POST Object restore] request to restore the object to a retrievable state.
...
If you are using xref:../admin/grid-federation-overview.adoc[grid federation] and xref:../tenant/grid-federation-manage-cross-grid-replication.adoc[cross-grid replication] is enabled for a bucket, the S3 client can verify an object's replication status by issuing a GET Object request. The response includes the StorageGRID-specific `x-ntap-sg-cgr-replication-state` response header, which will have one of the following values:
