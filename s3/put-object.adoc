...
*Attention:* The encryption keys you provide are never stored. If you lose an encryption key, you lose the corresponding object. Before using customer-provided keys to secure object data, review the considerations for link:using-server-side-encryption.html[using server-side encryption].
...
As a general best practice, you should always include these headers within `CanonicalHeaders` to ensure they are verified; however, if you exclude these headers, StorageGRID does not return an error.

IMPORTANT: If you use HTTPS, the StorageGRID behavior presents no security concern. If you use HTTP, a replay attack is potentially possible if you do not include these three types of headers in `CanonicalHeaders`.
