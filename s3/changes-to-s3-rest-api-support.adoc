...
* Added support for the StorageGRID-specific `x-ntap-sg-cgr-replication-state` response header for GET Object and HEAD Object requests. This header provides an object's replication status for cross-grid replication.
