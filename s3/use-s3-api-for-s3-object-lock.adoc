...
* View the bucket in the Tenant Manager. See xref:../tenant/viewing-s3-bucket.adoc[View S3 buckets].
...
User who have the `s3:BypassGovernanceRetention` permission can bypass the active retention settings of an object that uses GOVERNANCE mode. Any DELETE or PUT Object retention operations must include the `x-amz-bypass-governance-retention:true` request header. These users can perform these additional operations:
