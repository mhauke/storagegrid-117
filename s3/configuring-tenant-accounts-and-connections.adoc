...
If the *Enable HTTP for Storage Node connections* check box is selected, clients must use different ports for HTTP than they use for HTTPS. See the link:../admin/index.html[instructions for administering StorageGRID].
...
. Create a bucket.
+
This example assumes you configured your load balancer endpoint to use IP address 10.96.101.17 and port 10443.
+
----
