...
sidebar: sidebar
keywords: using S3 object lock, object lock, default bucket retention, governance, compliance
...
** In COMPLIANCE mode, an object version cannot be deleted until its retention period has elapsed. An object's retention period can be extended, but it cannot be reduced or removed.  

** In GOVERNANCE mode, users with special permission can bypass certain retention settings. They can delete an object version before its retention period has elapsed, or they can increase, decrease, or remove an object's retention period.

* *Default retention period*: Each bucket can have a default retention period in years or days. 
