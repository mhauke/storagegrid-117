...
When xref:../swift/configuring-tenant-accounts-and-connections.adoc[creating a Swift tenant account], the grid administrator specifies the following information:

* xref:../admin/editing-tenant-account.adoc[Display name for the tenant] (the tenant's account ID is assigned automatically and cannot be changed)
* Optionally, a xref:../admin/editing-tenant-account.adoc[storage quota for the tenant account] -- the maximum number of gigabytes, terabytes, or petabytes available for the tenant's objects. A tenant's storage quota represents a logical amount (object size), not a physical amount (size on disk).
* If xref:../admin/configuring-sso.adoc[single sign-on (SSO)] is not in use for the StorageGRID system, whether the tenant account will use its own identity source or share the grid's identity source, and the initial password for the tenant's local root user.
...
IMPORTANT: Swift users must have the Root access permission to xref:../tenant/signing-in-to-tenant-manager.adoc[access the Tenant Manager]. However, the Root access permission does not allow users to authenticate into the Swift REST API to create containers and ingest objects. Users must have the Swift Administrator permission to authenticate into the Swift REST API.
...
See xref:../admin/configuration-options-for-ha-groups.adoc[Configuration options for HA groups] for more information.
...
The table summarizes the different ways that clients can connect to StorageGRID and the IP addresses and ports that are used for each type of connection. See xref:../admin/summary-ip-addresses-and-ports-for-client-connections.adoc[IP addresses and ports for client connections] or contact your StorageGRID administrator for more information.
...
By default, when client applications connect to Storage Nodes, they must use encrypted HTTPS for all connections. Optionally, you can enable less-secure HTTP connections by selecting the *Enable HTTP for Storage Node connections* grid option in the Grid Manager. For example, a client application might use HTTP when testing the connection to a Storage Node in a non-production environment.

IMPORTANT: Be careful when enabling HTTP for a production grid since requests will be sent unencrypted.

If the xref:../admin/changing-network-options-object-encryption.adoc[*Enable HTTP for Storage Node connections] option is selected, clients must use different ports for HTTP than they use for HTTPS.
