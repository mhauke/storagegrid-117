...
* link:../admin/editing-tenant-account.html[Display name for the tenant] (the tenant's account ID is assigned automatically and cannot be changed)
...
IMPORTANT: Be careful when enabling HTTP for a production grid since requests and responses will be sent unencrypted.

If the link:../admin/changing-network-options-object-encryption.html[*Enable HTTP for Storage Node connections*] option is selected, clients must use different ports for HTTP than they use for HTTPS.
