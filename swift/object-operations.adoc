...
[.lead]
The following Swift API operations are performed on objects. These operations can be tracked in the xref:monitoring-and-auditing-operations.adoc[StorageGRID audit log].
...
For more information, see xref:../ilm/how-objects-are-deleted.adoc[How objects are deleted].
...
+
For more information about synchronous placement and ILM rules, see xref:../ilm/data-protection-options-for-ingest.adoc[Data-protection options for ingest].
